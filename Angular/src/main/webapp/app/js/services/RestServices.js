...
angularPOC.factory('RestServices', ['$http', 'ShareDataService', function($http, ShareDataService) {
	...
	var service = {
		...
		mockHello : function(successCallback, failCallback) {
...
//				MessageController.setMessage(data.description, data.responseStatus);
				if (data.responseStatus == "OK") {
					...
					console.log("success, but responseStatus was: " + data.responseStatus);
					console.log("ErrorDescription: " + data.description);
					failCallback();
				}
			}).error(function(status) {
				console.log("error: " + JSON.stringify(status));
		...
		getSinglePerson : function(successCallback, failCallback, id) {

			$http({
				url : 'http://localhost:8080/jeer/rest/person?personId='+id+"&phoneNumbers=true",
				dataType : 'json',
				method : 'GET',
				data : '',
				headers : {
		...
		getPersonPhoneNumbers : function(successCallback, failCallback, id) {
		...
		addPerson : function(successCallback, failCallback, data) {
		...
		savePersonChanges : function(successCallback, failCallback, data) {
		...
		getEmptyPerson : function(successCallback, failCallback) {
		...
		addPhoneNumber : function(successCallback, failCallback, data, personId) {
			
			$http({
				url : 'http://localhost:8080/jeer/rest/phonenumber?personId='+personId,
				dataType : 'json',
				method : 'POST',
				data : data,
				headers : {
        ...
        removePhoneNumber : function(successCallback, failCallback, id) {
        	
			$http({
				url : 'http://localhost:8080/jeer/rest/phonenumber?numberId='+id,
				dataType : 'json',
				method : 'DELETE',
				data : '',
				headers : {
		...
		getPhonenumberTypes : function(successCallback, failCallback) {
